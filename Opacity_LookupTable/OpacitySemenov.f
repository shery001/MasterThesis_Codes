...
C Von http://www2.mpia-hd.mpg.de/home/henning/Dust_opacities/Opacities/opacities.html
C heruntergeladen und von Gabriel-Dominique Marleau angepaßt
C 
C  Änderungen:
C   - Gasopazitäten ausgeschaltet (zwei Stellen ausgeschaltet)
C   - Evaporation der letzten Komponente (die bis ~1500 K überlebt)
C     kann jetzt extern berechnet werden; dafür EVAPMOSTREFRAC auf .false. setzen
C   - Gibt Opazität pro Gramm *Staub* wieder (siehe rhoStaub in semenov03_staubig_init)
C  
C  Zu tun: Staub- (für Evaporation) und Strahlungstemperatur (für Opazität)
C          eingeben und benutzen
C 
C MPIA Heidelberg, Uni Bern; 2015--2017
C 
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C     program  opacity.f
C     .........................................................................
...
! ! !       END
      ...
      SUBROUTINE semenov03_staubig_init(modeltopshape)
      
      IMPLICIT NONE
      
      REAL*8 eD_Ross, eD_Planck, rhoStaub
      DIMENSION eD_Ross(5,6),eD_Planck(5,6)
      CHARACTER*7 modeltopshape
      CHARACTER*3 model
      CHARACTER*1 top
      CHARACTER*1 shape
      LOGICAL ross
      
      COMMON /semenovkappa/ eD_Ross, eD_Planck, rhoStaub
      SAVE /semenovkappa/
      
      model = modeltopshape(1:3)
      top = modeltopshape(5:5)
      shape = modeltopshape(7:7)
      
c      print *, ' * Init von Semenov:  model,top,shape:  ',
c     &          trim(model),',',trim(top),',',trim(shape)
      
      ! Summen der f_i-Spalten der Tabelle 1 von Semenov et al. (2003):
      !  2.64e-3 + 1.26e-4 + 7.70e-4 + 7.68e-4 + 3.53e-4 + 6.02e-4 + 5.55e-3 = 0.010809
      !  3.84e-3 + 0e-4    + 4.44e-4 + 3.80e-4 + 3.53e-4 + 6.02e-4 + 5.55e-3 = 0.011169
      !  6.30e-3 + 7.97e-4 + 1.91e-3 + 7.68e-4 + 3.53e-4 + 6.02e-4 + 5.55e-3 = 0.016280
      if(model.eq.'nrm') rhoStaub = 0.010809D0
      if(model.eq.'irs') rhoStaub = 0.011169D0
      if(model.eq.'ips') rhoStaub = 0.016280D0
      
      ross = .true.
      CALL init_d(ross,model,top,shape,eD_Ross)
      ross = .false.
      CALL init_d(ross,model,top,shape,eD_Planck)
      
      
      END SUBROUTINE semenov03_staubig_init
      
! ------------------------------------------------------------------------
      
      SUBROUTINE semenov03_staubig(Typ, rho, T, kappaStaub)
      
      INTEGER Typ
      REAL*8 rho, T, kappaStaub, nichts
      REAL*8 eD_Ross, eD_Planck, rhoStaub
      DIMENSION eD_Ross(5,6),eD_Planck(5,6)
      ...
      if ( Typ.eq.1) then
         CALL cop(eD_Ross,nichts,rho,T, kappaStaub)
      
      else if ( Typ.eq.2 ) then
         CALL cop(eD_Planck,nichts,rho,T, kappaStaub)
      
      else
         print *, ' !! Unbekanntes Opazitaetstyp in semenov03_staubig: ',Typ,'. Gueltig: 1 (=Rosseland) oder 2 (=Planck) !!'
         STOP
      endif
      
      ! this is now the opacity per Gram of dust
      kappaStaub = kappaStaub/rhoStaub
      
      return
      END SUBROUTINE semenov03_staubig
...
C Global variable(s):
      ...
      LOGICAL ross
c      if(ross)      print *, 'Rosseland '
c      if(.not.ross) print *, 'Planck    '
...
C.............................................................................
      DO I = 1,4
         DO J = 1,8
            temp(J) = Tt(J,I)
         END DO
           ! lineare Extrapolation ist nicht gut;
           !   ergibt z.B. 580 K für H2O bei rho=1e-4 usw.
           ! log ist aber teuer -> einfaches Maximum
!            IF(rho_in.gt.ro(8).or.rho_in.lt.ro(1))THEN
!             CALL bint(dlog10(rho_in),8,dlog10(ro),T_ev(I),dlog10(temp))
!             T_ev(I) = 10.0d0**T_ev(I)
!            ELSE
             CALL bint(rho_in,8,ro,T_ev(I),temp)
!            ENDIF
           ! 265: muß kleiner als T(2) = 275 unten sein
           if(I.eq.1) T_ev(I) = DMAX1(DMIN1( 265.0D0,T_ev(I)), 100.0D0)
           if(I.ge.2) T_ev(I) = DMAX1(DMIN1(2000.0D0,T_ev(I)), 777.0D0)
      END DO
C.............................................................................
C  Set up the evaporation temperature array 'T(1:5)':
C.............................................................................    
c         Evaporation of the most refractory component: compute here
c         (as in original routine)? If .FALSE., should do it in DustEvolution.c
          EVAPMOSTREFRAC = .TRUE.
...
C.............................................................................
       IF (KK.eq.6) THEN
c        keine Gasopazität:
c        CALL gop(eG,rho_in,T_in,aKext)
        aKext = 0.0D0
        RETURN
       END IF
C.............................................................................
...
C
       IF (KK.eq.5) THEN
c        keine Gasopazität:
c        CALL gop(eG,rho_in,T2,aKg_ext)
      aKg_ext = 0.0D0
      aKrR = aKg_ext
       ELSE
...
C.............................................................................
     ...
     +                eD(KK,4))*T_in+eD(KK,5))*T_in+eD(KK,6)
          aKext = DMAX1(aKext, 0.0D0)  ! wenn T(5) ~ unendlich, Polynom kann <0 werden
       END IF
C Exit:
      RETURN
      END
...
c       print *,'Dust model: "normal" silicates; homogeneous spheres'
...
c       print *,'Dust model: "normal" silicates; homogeneous dust',
c     &                                          ' aggregates'
...
c       print *,'Dust model: "normal" silicates; composite dust',
c     &                                          ' aggregates'
...
c       print *,'Dust model: "iron-rich" silicates; homogeneous spheres'
...
c       print *,'Dust model: "iron-rich" silicates; homogeneous dust',
c     &                                            ' aggregates'
...
c       print *,'Dust model: "iron-rich" silicates; composite dust',
c     &                                             ' aggregates'
...
c       print *,'Dust model: "iron-poor" silicates; homogeneous spheres'
...
c       print *,'Dust model: "iron-poor" silicates; homogeneous dust',
c     &                                             ' aggregates'
...
c       print *,'Dust model: "iron-poor" silicates; composite',
c     &         ' aggregates'
...
c       print *,'Dust model: "iron-rich" silicates; porous',
c     &         ' composite spherical particles'
C Set up output parameters:
       DO i = 1,5
         DO j = 1,6
...
c       print *,'Dust model: "normal" silicates; composite',
...
c       print *,'Dust model: "normal" silicates; porous',
c     &         ' composite spheres'
C Set up output parameters:
       DO i = 1,5
         DO j = 1,6
...
c       print *,'Dust model: "iron-rich" silicates; composite',
c     &         ' spheres'
C Set up output parameters:
       DO i = 1,5
         DO j = 1,6
...
c       print *,'Dust model: "iron-rich" silicates; compact',
c     &         ' 5-layered spheres'
C Set up output parameters:
       DO i = 1,5
         DO j = 1,6
...
c       print *,'Dust model: "iron-poor" silicates; porous',
c     &         ' 5-layered spheres'
C Set up output parameters:
       DO i = 1,5
         DO j = 1,6
